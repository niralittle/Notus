...
List<Map<String, String>> tasksEngFull =
            (List<Map<String, String>>) request.getAttribute("tasksEngFull");
...
%>
<h3>
    <%=title + " "%> tasks assignment for <%=user.getFirstName() + " " + user.getLastName()%>
</h3>
<%
...
<%
    if (request.getAttribute("noOfPages") != null &&
        ...
        if (noOfPages > 1) {
            ...
            String url = personal ? "TasksAssignment?type=personal&" :
                                    "TasksAssignment?";
...
%>
        <li<%=currentPage < noOfPages ? "" : " class=\"disabled\""%>>
            <a href="<%=currentPage < noOfPages ? url + "page=" + (currentPage+1) : "#"%>">&raquo;</a>
        </li>
<%
